...
export const createJadwal = async (req, res) => {
  try {
    ...
    const skedul = await prisma.jadwal.findMany({ where: { id: jadwal.id }, include: { mataKuliah: true, dosen: true}})

    const respon = skedul.map((ja)=> {
      const k = {
        id: ja.id,
        mataKuliahId: ja.mataKuliah.nama,
        Pengajar: ja.dosen.nama,
        hari: ja.hari,
        jamMulai: ja.jamMulai,
        jamSelesai: ja.jamSelesai
      }
      return k;
    })
    res.status(201).json({
...
export const updateJadwal = async (req, res) => {
  try {
    ...
    const perbarui = await prisma.jadwal.findMany({ where: { id: updated.id }, include: { mataKuliah: true, dosen: true}})
  const respon = perbarui.map((ka)=> {
    const q = {
      id: ka.id,
        mataKuliahId: ka.mataKuliah.nama,
        Pengajar: ka.dosen.nama,
...
export const deleteJadwal = async (req, res) => {
  try {
    ...
    const hilang = await prisma.jadwal.findMany({ where: { id: id.id }, include: { mataKuliah: true, dosen: true}})

    const hapus = hilang.map((ag)=> {
      const v = {
        id: ag.id,
        mataKuliahId: ag.mataKuliah.nama,
        Pengajar: ag.dosen.nama,
        hari: ag.hari,
        jamMulai: ag.jamMulai,
        jamSelesai: ag.jamSelesai,
